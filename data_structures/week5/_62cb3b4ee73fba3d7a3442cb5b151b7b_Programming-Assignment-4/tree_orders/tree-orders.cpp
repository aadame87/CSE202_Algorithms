...
public:
  ...
  void InOrderTraversal(vector<int> &result, int index){
    if (index == -1) return;
  ...
  vector <int> in_order() {
    ...
    if (key.empty()) return vector<int>();
    InOrderTraversal(result,0);
    return result;
  }

  void PreOrderTraversal(vector<int> &result, int index){
    if (index ==-1) return;
...
void PostOrderTraversal(vector<int> &result, int index){
  if (index==-1) return;
