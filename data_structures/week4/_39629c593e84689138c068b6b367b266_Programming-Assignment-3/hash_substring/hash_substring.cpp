...
std::vector<long long> precomputeHashes(const string& T, const string& P, long long p, long long x) {
std::vector<long long> H(T.size()-P.size()+1);
string S = T.substr(T.size()-P.size(), P.size());
...
for ( long long i = ( T.size() - P.size() - 1 ); i >= 0; --i ) {
